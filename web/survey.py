...
class Thanks:
    ...
    def POST(self):
        ...
        web_path = "/var/www/AppRecommender/src/web/submissions/%s/personal"
        personal_file = open(web_path % user_id, 'w')
        personal = {}
    ...
    def POST(self):
        ...
        try:
            prediction_file = open(
                os.path.join(strategy_dir, "prediction"), 'r')
            evaluation_file = open(
                os.path.join(strategy_dir, "evaluation"), 'w')
            reader = csv.DictReader(prediction_file)
            writer = csv.DictWriter(
                evaluation_file, fieldnames=reader.fieldnames)
        ...
        try:
            ...
            if "comments" in web_input:
                comments = web_input['comments'].encode("utf-8")
            else:
                comments = ""
            writer.writerow(
                (user_id, strategy, start_time, end_time, summary["poor"],
        ...
        if 'continue_button' in web_input:
            return Survey().POST()
        elif 'finish_button' in web_input:
...
render = web.template.render(
    '/var/www/AppRecommender/src/web/templates/', base='layout',
    globals={'hasattr': hasattr})
render_plain = web.template.render(
    '/var/www/AppRecommender/src/web/templates/', globals={'hasattr': hasattr})
