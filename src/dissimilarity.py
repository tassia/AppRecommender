...
class CosineDissimilarity(Dissimilarity):
    """
    Dissimilarity measure complementary to the cosine similarity which is defined by the
...
class JaccardDistance(Dissimilarity):
    """
    Dissimilarity measure complentary to Jaccard Index which is defined by the quantity of
    common values divided by the size of the two sets union.
    """
    def __call__(self,x,y):
        """
...
class DiffCoefficient(Dissimilarity):
    """
    Measure the difference between the two sets in terms of how many items should be added and
    removed from one set to transform it into the other set. Similar to edit distance, but the
    items positions are not relevant for sets.
    """
    def __call__(self,x,y):
        """
